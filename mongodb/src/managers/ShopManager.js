...
class ShopManager extends Emitter {
    ...
    async buy(itemID, memberID, guildID, quantity = 1, currency = null, reason = 'received the item from the shop') {
        ...
        if (
            ...
            item: new ShopItem(guildID, memberID, this.options, item, this.database, this.cache),
            quantity,
            totalPrice,
            currencyUsed: currency ? await this.currencies.get(currency, guildID) : null
        }
        ...
        if (savePurchasesHistory) {
            ...
            await this.database.push(`${guildID}.${memberID}.history`, {
                ...
                custom: item.custom || {},
                currencyUsed: currency ? (await this.currencies.get(currency, guildID)).rawObject : null
            })
