...
class InventoryManager extends Emitter {
     ...
     */
    async addItem(itemID, memberID, guildID, quantity = 1, currency = null) {
        ...
        */
        ...
        if (!item) return {
            ...
            quantity: 0,
            totalPrice: 0,
            currencyUsed: currency ? await this.currencies.get(currency, guildID) : null
        }
     ...
     */
    async sellItem(itemID, memberID, guildID, quantity = 1, currency = null, reason = 'sold the item from inventory') {
        const inventory = await this.fetch(memberID, guildID)
