...
class ShopItem extends Emitter {
    ...
    async buy(memberID, quantity = 1, currency = null, reason = 'received the item from the shop') {
        ...
        if (!item) return {
            ...
            quantity: 0,
            totalPrice: 0,
            currencyUsed: currency ? await this.currencies.get(currency, this.guildID) : null
        }
        ...
        if (savePurchasesHistory) {
            ...
            await this.database.push(`${this.guildID}.${memberID}.history`, {
                ...
                custom: item.custom || {},
                currencyUsed: currency ? (await this.currencies.get(currency, this.guildID)).rawObject : null
            })
